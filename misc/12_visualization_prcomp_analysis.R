...
loaded$log2norm.prcomp.centred <- prcomp(t(loaded$log2norm$counts), center = TRUE, scale. = FALSE)
loaded$log2norm.prcomp.uncentred <- prcomp(t(loaded$log2norm$counts), center = FALSE, scale. = FALSE)
...
names(loaded$log2norm.prcomp.centred)
head(loaded$log2norm.prcomp.centred$center)
head(loaded$log2norm.prcomp.centred$scale)
dim(loaded$log2norm.prcomp.centred$rotation)
loaded$log2norm.prcomp.centred$rotation[1:10, 1:5] ## This corresponds to the loadings, i.e. the weight of each variable (gene) on each component
loaded$log2norm.prcomp.centred$x[1:10, 1:5] ## x corresponds to the "scores", or coordinates of each individual (sample) on the components
...
pdf(file = PC.sd.plot)
barplot(loaded$log2norm.prcomp.centred$sdev,
...
par(mfrow = c(2,1))
barplot((loaded$log2norm.prcomp.centred$sdev^2)[1:50], las=1, ylab="Variance per coponent", xlab="Component number", main="Centered variables")
barplot((loaded$log2norm.prcomp.uncentred$sdev^2)[1:50], las=1, ylab="Variance per coponent", xlab="Component number", main="Uncentered variables")
par(mfrow = c(1,1))
...
ymin <- PC2.min <- min(loaded$log2norm.prcomp.centred$x[,2])
PC2.max <- max(loaded$log2norm.prcomp.centred$x[,2])
ymax <- PC2.max + 0.4*(PC2.max - PC2.min)
plot(loaded$log2norm.prcomp.centred$x[,c(1,2)],
     col=sampleColors,
...
plot(loaded$log2norm.prcomp.centred$x[,c(3,4)],
     col=sampleColors,
     main=paste0("PC plot for the experiment no._", parameters$recountID), las=1, panel.first=grid())

plot(loaded$log2norm.prcomp.centred$x[,c(1,3)],
     col=sampleColors,
     main=paste0("PC plot for the experiment no._", parameters$recountID), las=1, panel.first=grid())

plot(loaded$log2norm.prcomp.centred$x[,c(1,4)],
     col=sampleColors,
     main= paste0("PC plot for the experiment no._", parameters$recountID), las=1, panel.first=grid())
...
scatterplot3d(x = loaded$log2norm.prcomp.centred$x[,3],
              y = loaded$log2norm.prcomp.centred$x[,4],
              z = loaded$log2norm.prcomp.centred$x[,1], color=sampleColors,
...
# library(rgl)
# plot3d(x = loaded$log2norm.prcomp.centred$x[,1],
#               y = loaded$log2norm.prcomp.centred$x[,2],
#               z = loaded$log2norm.prcomp.centred$x[,3], col = sampleColors,
...
# library(plotly)
# p <- plot_ly(as.data.frame(loaded$log2norm.prcomp.centred$x[,1:3]))
...
log2normCounts.components <- loaded$log2norm.prcomp.centred$x[,1:nb.components]
